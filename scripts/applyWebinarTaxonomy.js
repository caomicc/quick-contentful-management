...
async function main() {
  const { mappings } = JSON.parse(fs.readFileSync('webinar_taxonomy_mapping.json', 'utf8'));
  const client = contentful.createClient({ accessToken: process.env.CONTENTFUL_MANAGEMENT_TOKEN });
  const space = await client.getSpace(process.env.CONTENTFUL_SPACE_ID);
  const env = await space.getEnvironment('master');

  // Fetch all published webinars (using 'sys.publishedAt[exists]=true' for reliable filter)
  const entries = [];
  let skip = 0;
  while (true) {
  ...
  // Use all entries since we filtered for published in the query
  const published = entries;
  console.log(`Found ${published.length} published webinars\n`);
  ...
  console.log(`Mapping has ${mappings.length} entries, ${published.length} published webinars.`);
  if (DRY_RUN) {
